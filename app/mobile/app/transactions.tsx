    ...
    <View style={[skeleton.row, { borderBottomColor: theme.border }]}>
      ...
      <View style={skeleton.lines}>
        <View
          style={[
            skeleton.line,
            { width: "55%", backgroundColor: theme.skeleton },
          ]}
        />
        <View
          style={[
            skeleton.line,
            { width: "35%", marginTop: 6, backgroundColor: theme.skeleton },
          ]}
        />
      </View>
      <View
        style={[
          skeleton.line,
          { width: 60, alignSelf: "center", backgroundColor: theme.skeleton },
        ]}
      />
    </View>
  );
...
export default function TransactionsScreen() {
  ...
  // Pass server-side asset filter to the hook for efficient backend queries
  const serverAsset = assetFilter === "All" ? undefined : assetFilter;
  const {
    transactions,
    loading,
    refreshing,
    ...
    refresh,
    loadMore,
  } = useTransactions(accountId, { asset: serverAsset });
    ...
    <View style={styles.listHeader}>
      ...
      {staleCache ? (
        ...
        >
          ...
          />
          <Text style={[styles.staleBannerText, { color: theme.textMuted }]}>
            Showing cached data. Pull to refresh for live data.
          </Text>
        </View>
      ) : null}
      ...
      <View style={styles.headerRow}>
        <Text
          style={[
            styles.accountPill,
            { backgroundColor: theme.border, color: theme.textPrimary },
          ]}
        >
          {shortAccount}
        </Text>
